...
export default function HeroSection({
  ...
  return (
    <section
      ref={heroRef}
      className="relative h-screen w-full text-white font-(--font-montserrat)"
    >
              ...
              <form onSubmit={handleSearchSubmit} className="relative">
                <div className="flex items-center w-full bg-white rounded-full shadow-lg overflow-hidden h-12.5">
                  <div className="relative flex-1 h-full">
                    <input
                      ...
                      className="w-full h-full px-6 text-[15px] text-gray-700 bg-transparent outline-none font-medium"
                    />
                    {!searchQuery && (
                  ...
                  <button
                    type="submit"
                    className="flex items-center justify-center w-10 h-10 mr-1.25 rounded-full transition hover:scale-95"
                  >
                    <svg width="34" height="34" viewBox="0 0 34 34" fill="none">
                      ...
                      />
                      <path
                        d="M16.5 24C20.37 24 23.5 20.87 23.5 17S20.37 10 16.5 10 9.5 13.13 9.5 17s3.13 7 7 7Z"
                        stroke="#003F6B"
                        strokeWidth="1.5"
                        strokeLinecap="round"
                      ...
                      />
                      <path
                        d="M16.5 13c1.06 0 2.08.42 2.83 1.17.75.75 1.17 1.77 1.17 2.83"
                        stroke="#003F6B"
                        strokeWidth="1.5"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                ...
                {searchSuggestions.length > 0 && (
                  <div className="absolute top-full left-0 right-0 mt-2 rounded-2xl bg-white border border-gray-100 p-2 shadow-xl z-20 text-slate-800 text-left">
                    <span className="block text-[10px] uppercase font-bold text-slate-400 tracking-wider px-3 py-1">
                      Matching Services
                    </span>
                    {searchSuggestions.map((s, i) => (
                      <button
                        ...
                        className="w-full text-left rounded-lg px-3 py-2 text-sm hover:bg-slate-50 font-medium transition-colors"
                      >
                        {s}
                      </button>
                    ))}
                ...
                {searchSubmitted && (
                  <div className="absolute top-full left-0 right-0 mt-3 rounded-xl bg-emerald-500/90 backdrop-blur-md px-4 py-2.5 text-xs text-white z-20 text-center shadow-lg font-semibold">
                    ⚡ Search active! Generating matches for &quot;{searchQuery}
                    &quot; near you...
                  </div>
                )}
              </form>
