...
export default function HeroSection({ heroRef }: { heroRef: React.RefObject<HTMLElement | null> }) {
  ...
  useEffect(() => {
    const word = SERVICES_LOOP[wordIdx];
    if (isExiting) {
      const t = setTimeout(() => { setIsExiting(false); setDisplayText(""); setWordIdx(p => (p + 1) % SERVICES_LOOP.length); }, 400);
      return () => clearTimeout(t);
    }
    if (displayText.length < word.length) {
      const t = setTimeout(() => setDisplayText(word.substring(0, displayText.length + 1)), 65);
      return () => clearTimeout(t);
    }
    const t = setTimeout(() => setIsExiting(true), 2000);
    return () => clearTimeout(t);
  }, [displayText, isExiting, wordIdx]);
  ...
  return (
    <section ref={heroRef} className="relative h-screen w-full text-white font-[var(--font-montserrat)]">
      <div className="w-full h-full flex items-center justify-center">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center space-y-3 z-10 px-4">
          <div className="w-full pt-4 transition-all duration-1000 ease-out opacity-100 translate-y-0" style={{ transitionDelay: "100ms" }}>
            <h1 className="font-bold text-white flex flex-col md:flex-row items-center justify-center gap-x-3 gap-y-2 leading-tight text-center">
              ...
              <span className="md:text-left text-3xl md:text-4xl lg:text-5xl xl:text-6xl inline-block overflow-hidden">
                <span className="inline-block font-bold text-amber-400 transition-transform duration-[380ms] ease-in-out"
                  style={{ transform: isExiting ? "translateY(110%)" : "translateY(0%)" }}>
          ...
          <div className="w-full flex flex-col items-center gap-4 pt-4">
            <div className="px-3 w-full max-w-xl md:max-w-[680px]">
              <form onSubmit={handleSearchSubmit} className="relative">
                <div className="flex items-center w-full bg-white rounded-full shadow-lg overflow-hidden h-[50px]">
                  ...
                  <button type="submit" className="flex items-center justify-center w-[40px] h-[40px] mr-[5px] rounded-full transition hover:scale-95">
